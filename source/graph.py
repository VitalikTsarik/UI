...
class Graph(object):
    ...
    def add_edge(self, idx, length, vert_from, vert_to):
        ...
        if vert_from in self.__graph.keys():
            for vertex in self.__graph[vert_from]['adj_edges']:
                if vertex['vert_to'] == vert_to:
                    return
        else:
            self.add_vertex(vert_from)

        self.__graph[vert_from]['adj_edges'].append(
            {
                'edge_idx': idx,
                'length': length,
                'vert_to': vert_to,
        ...
        self.__graph[vert_to]['adj_edges'].append(
            {
                'edge_idx': idx,
                'length': length,
                'vert_to': vert_from,
    ...
    def get_adj_vertices(self, idx):
        return tuple([edge['vert_to'] for edge in self.__graph[idx]['adj_edges']])
    ...
    def get_edge_by_idx(self, idx):
        for vert in self.__graph.keys():
            for edge in self.__graph[vert]['adj_edges']:
    ...
    def get_edge_by_adj_vert(self, vert1, vert2):
        for edge in self.__graph[vert1]['adj_edges']:
